...
# Update — GET /api/update-start  (EventSource-compatible alternative)
@ipc_fast_api.get("/api/update-start")
async def start_update_get(
    command        : str           = Query(...),
    sudo_password  : str           = Query(...),
    components     : Optional[str] = Query(None),
    current_user   : dict          = Depends(require_role("admin")),
    befa                           = Depends(get_be_fastapi_obj)
):
    log_debug(f"FAPI start update (GET) requested by {current_user['user_uid']}")

    try:
        parsed_components = json.loads(components) if components else None
    except Exception:
        return fapi_error_response("update", "Invalid components format")
    ...
    if not status:
        log_error(f"FAPI start update (GET) failed: {result}")
        return fapi_error_response("update", "Update failed", result)
    ...
    async def stream():
        try:
            ...
            log_debug("FAPI update stream (GET) cancelled by client")

        except Exception as e:
            log_error(f"FAPI update stream (GET) error: {e}")
            yield f"data: {json.dumps({'type': 'error', 'message': str(e)})}\n\n"
            yield "data: [DONE]\n\n"
